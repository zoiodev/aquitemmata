...
class AppController extends Controller {
	...
	public $components = array(
        ...
        'Session',
        'Auth' => array(
            'loginAction' => array('controller' => 'index', 'action' => 'index', 'admin' => false),
	...
	function beforeFilter(){
        ...
        if(!empty($this->params['prefix']) && $this->params['prefix'] == 'admin'){
			...
			if ($this->Session->read('Auth.User')):
				...
				if ($this->Auth->user('role_id') != 1):
					if($this->action == 'admin_index' && $this->Auth->user('role_id') == 3 ){
						$this->redirect(array('controller' => 'index', 'action' => 'index', 'sos_admin' => true));
					}
		...
		/// <==========ADMIN
		if($this->params['controller'] != 'user' && $this->Auth->user('role_id') == ''){
			if($this->layout != 'default'){
				$this->redirect(array('controller' => 'user', 'action' => 'login', 'admin' => true));
			}
		}
		...
		if(!empty($this->params['prefix']) && $this->params['prefix'] == 'sos_admin') {
			...
			if($this->action == 'sos_admin_index' && $this->Auth->user('role_id') != 3){
...
*/
	...
	public function sos_admin_leituraAndEditJson($jsonNomeSemExt=null, $jsonFormat=null, $actionRedirect=null) {
		...
		$pathBk = $_SERVER [ 'DOCUMENT_ROOT' ].$this->webroot.'app/webroot/json/'.$jsonNomeSemExt.'/bk/';//<<< caminho absoluto da pasta
		$pathSOS = $_SERVER [ 'DOCUMENT_ROOT' ].'/aquitemmata.com.br/www/json/'.$jsonNomeSemExt.'/';//<<< caminho absoluto da pasta
		$pathSOSBk = $_SERVER [ 'DOCUMENT_ROOT' ].'/aquitemmata.com.br/www/json/'.$jsonNomeSemExt.'/bk/';//<<< caminho absoluto da pasta
			...
			foreach ($files as $file){
			    ...
			    if($file->name == $seachName){
			    	if($this->request->is('post')){
						...
						//>>> no momento da alteração, criar a pasta BK caso a mesma não exista
						if(is_dir($pathBk) !== true):
							$file = new Folder($pathBk, true, 0777);//<<< Criar diretorio de Bk
						endif;
						if(is_dir($pathSOSBk) !== true):
							$fileSOS = new Folder($pathSOSBk, true, 0777);//<<< Criar diretorio de Bk
						endif;
						//<<< no momento da alteração, criar a pasta BK caso a mesma não exista

				    	rename($path.$seachName, $path.'bk/'.$nameFile.'_bk_'.date('Ymd_h\hi\ms\s').'.json');//<<< move e renomeia o arquivo
				    	rename($pathSOS.$seachName, $pathSOS.'bk/'.$nameFile.'_bk_'.date('Ymd_h\hi\ms\s').'.json');//<<< move e renomeia o arquivo

						$file = new File($path.$seachName, true, 0777);//<<< seta arquivo
					    $contents = $file->read();//<<< abre a leitura
					    $file->write($formatJson);//<<< escreve o json
					    $file->close(); //<<< fecha leitura

						$fileSOS = new File($pathSOS.$seachName, true, 0777);//<<< seta arquivo
					    $contentsSOS = $fileSOS->read();//<<< abre a leitura
					    $fileSOS->write($formatJson);//<<< escreve o json
					    $fileSOS->close(); //<<< fecha leitura
				    	...
				    	$fileSOS = new File($pathSOS.$seachName, true, 0644);//<<< criar arquivo
					    $contents = $fileSOS->read();
					    $contentHome = json_decode($contents, true);//<<< leitura do json
					    $fileSOS->close(); //<<< fecha leitura
	...
	public function recuperarInfoJson($jsonNameSemExt=null){
		$path = $_SERVER [ 'DOCUMENT_ROOT' ].'/SOSMA/www/json/'.$jsonNomeSemExt.'/';//<<< caminho absoluto da pasta
