...
class AppController extends Controller {
	...
	function beforeFilter(){
		$this->Auth->authError = __('Você não possui altorização.');
		
		if(!empty($this->params['prefix']) && $this->params['prefix'] == 'admin'){
			//>>> ADMIN =======>
			if($this->Auth->user('role_id') == 1 && $this->params['prefix'] != 'admin'){
				return $this->Auth->user('role_id') == 1;
			}
			if($this->params['prefix'] == 'admin' && $this->Auth->user('role_id') == ''){
				if($this->Auth->user('role_id') == 1){
					$this->layout = 'admin';
				}
				if($this->name == 'User' && $this->Auth->user('role_id') == ''){
					$this->layout = 'login';
				}
				$this->layout = 'login';
			...
			//>>> SOS_ADMIN =======>
			if($this->Auth->user('role_id') == 3 && $this->params['prefix'] != 'sos_admin'){
